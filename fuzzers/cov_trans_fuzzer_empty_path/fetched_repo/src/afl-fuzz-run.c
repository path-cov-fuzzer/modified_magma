...
// CYHADDED: 包含 path_reduction.h 头文件 ------------ start
...
fuzz_run_target(afl_state_t *afl, afl_forkserver_t *fsrv, u32 timeout) {
  ...
  // CYHADDED: 拷贝一些 afl 阶段名 ------------------------------------ start
  extern u8 afl_stage_name[256];
  strcpy(afl_stage_name, afl->stage_name);
  // CYHADDED: 拷贝一些 afl 阶段名 ------------------------------------ end

  // CYHADDED: 如果一小时内 corpus count 没有变化，那么就切换到下一个 k ------------ start
  ...
  extern PathReducer* used_path_reducer;

  static u64 prev_execs = -1; // 之前运行到这里时的总执行数
  static u32 prev_queued_cov = -1; // 之前记录的 队列中 +cov 和 +pat 数量
...
abort_calibration:
  ...
  // CYHADDED: 为新的 path-cov 做出的改变
  if (new_bits == 3 && !q->has_new_cov) {
