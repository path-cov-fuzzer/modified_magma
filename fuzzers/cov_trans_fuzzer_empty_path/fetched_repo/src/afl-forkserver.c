...
// CYHADDED: 用来告知 afl-forkserver 目前 AFL_stage 的全局变量 --------------------------- start
u8 afl_stage_name[256];
// CYHADDED: 用来告知 afl-forkserver 目前 AFL_stage 的全局变量 --------------------------- end
...
// 供全局使用的 path_reducer
PathReducer* path_reducer3  = NULL;
PathReducer* path_reducer5  = NULL;
PathReducer* path_reducer9  = NULL;
PathReducer* path_reducer17 = NULL;
PathReducer* used_path_reducer = NULL;
// CYHADDED: path_fuzzing 以及 path_reducer --------------------------- end
...
afl_fsrv_run_target(afl_forkserver_t *fsrv, u32 timeout,
  ...
  // CYHADDED: 先用 Zekun code 对路径进行缩减，随后对共享内存中的数据计算哈希值，接着重置 counter --------------------------- start
  // 优化：在 "染色阶段", "校准阶段" 和 "裁剪阶段" 不需要计算 reduced_path
  if(NULL != used_path_reducer && 0 != strcmp(afl_stage_name, "calibration") && 0 != strcmp(afl_stage_name, "colorization") && 0 != strncmp(afl_stage_name, "trim", 4) ) {
    ...
    reduced_path = reduce_path1(used_path_reducer, path_shm_ptr_pointer_to_1, fsrv->path_trace_bits[0], 0, &out_len); // CYHTO_DO: 加上 Zekun reduction 后要取消注释
