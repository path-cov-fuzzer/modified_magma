...
// CYHADDED: Load_CFG，以及 Zekun_path_reduction 相关的 API ------------ start
#include <path_reduction.h>

typedef struct TopLevel TopLevel;

// 供全局使用的 path_reducer
extern PathReducer* path_reducer3;
extern PathReducer* path_reducer5;
extern PathReducer* path_reducer9;
extern PathReducer* path_reducer17;
// CYHADDED: Load_CFG，以及 Zekun_path_reduction 相关的 API ------------ end

static void at_exit() {

  s32   i, pid1 = 0, pid2 = 0, pgrp = -1;
  // CYHADDED: 加了 PATH_SHM_ENV_VAR
...
void dump_block(BlockEntry *block) {
    printf("block->calls = %d\n", block->calls);
    printf("block->successor_size = %d\n", block->successor_size);
...
void dump_cfg(CFG *cfg) {
    printf("cfg->function_name = %s\n", cfg->function_name);
    printf("cfg->entry = %d\n", cfg->entry);
    printf("cfg->exit = %d\n", cfg->exit);
}

void dump_top(TopLevel *top) {
    printf("top->cfg_size = %d\n", top->cfg_size);
    printf("top->block_size = %d\n", top->block_size);
...
TopLevel *load_top(const char *filename) {
    TopLevel *top = (TopLevel *)malloc(sizeof(TopLevel));
...
// CYHADDED: CFG相关结构体 ------------------------------------------------------------------- end

/* Main entry point */
int main(int argc, char **argv_orig, char **envp) {

  // CYHADDED: 在这里根据环境变量加载 CFG binary，同时获取 path_reducer ----------------------- start
  // 初始化用来加载 CFG binary 的 Top_Level 结构体
  TopLevel *top = NULL;
  // 根据环境变量获取 CFG binary 路径
  const char* cfg_filepath = getenv("CFG_BIN_FILE");
  // 加载 CFG binary
  printf("loading CFG binary: %s .......\n", cfg_filepath);
  extern TopLevel *load_top(const char *filename);
  top = load_top(cfg_filepath);
  // // dump CFG binary
  // extern void dump_top(TopLevel *top);
  // dump_top(top);
  // 获取 path_reducer
  printf("generating path reducer .......\n");
  extern PathReducer* get_path_reducer(const void* top_level, int32_t k);
  path_reducer3  = get_path_reducer(top, 3);
  path_reducer5  = get_path_reducer(top, 5);
  path_reducer9  = get_path_reducer(top, 9);
  path_reducer17 = get_path_reducer(top, 17);
  ...
  // CYHADDED: 由于某些特殊原因，需要在这里声明函数 ----------------------- start
  extern void top_free(TopLevel *top);
  extern void free_path_reducer(PathReducer* ptr);
  // CYHADDED: 由于某些特殊原因，需要在这里声明函数 ----------------------- end

stop_fuzzing:

  // CYHADDED: 在这里释放 CFG binary 以及 path_reducer ----------------------- start
  printf("free path reducer .........\n");
  free_path_reducer(path_reducer3);
  free_path_reducer(path_reducer5);
  free_path_reducer(path_reducer9);
  free_path_reducer(path_reducer17);
  path_reducer3  = NULL;
  path_reducer5  = NULL;
  path_reducer9  = NULL;
  path_reducer17 = NULL;
  ...
  top_free(top);
  top = NULL;
  // CYHADDED: 在这里释放 CFG binary 以及 path_reducer ----------------------- end
