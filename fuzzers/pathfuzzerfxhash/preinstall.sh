...
apt-get update && \
    apt-get install -y make clang-11 llvm-config-11-dev libc++-11-dev libc++abi-11-dev \
        build-essential git wget gcc-11-plugin-dev rustc linux-tools-common linux-tools-generic linux-cloud-tools-generic

update-alternatives \
  --install /usr/lib/llvm              llvm             /usr/lib/llvm-config-11  20 \
